...
<aside class="fixed left-0 top-0 h-screen w-[5em] border-r border-gray-200 bg-[#0242a6] shadow-md transition-all duration-300 ease-in-out lg:w-[17rem] flex flex-col">
    ...
    <section class="flex flex-col gap-6 px-6 text-gray-600 overflow-y-auto flex-1">
        <?php
        $sidebar_items = [
            ["gambar" => "/technorules/images/home.svg", "label" => "Dashboard", "url" => "/technorules/komdis"],
            ["gambar" => "/technorules/images/evaluate.svg", "label" => "Evaluate", "url" => "/technorules/komdis/evaluate"],
        ];
    ...
    <section class="flex flex-col px-4 pt-2 pb-8">
        <a href="/technorules/komdis/profile" class="flex items-center justify-center w-full gap-3 bg-[#0a97ff] px-4 py-2 rounded-lg cursor-pointer">
            <img src="/technorules/images/user.svg" alt="User" />
            <span class="text-white flex-1 hidden lg:block">
                <h5 class="font-bold"><?php echo $data['nama_dosen']; ?></h5>
                <h6 class="text-xs"><?php echo $data['jabatan_dosen']; ?></h6>
            </span>
        </a>
