    ...
    <tbody>
        <?php
            ...
            if (!empty($dataPelanggaran)) {
                    ...
                    echo "<tr>
                        ...
                        <td class='px-4 py-2 h-full border-b border-[#e4e8ed]'>
                            <div class='flex items-center justify-center gap-6'>
                                <i class='edit fa-solid fa-pen-to-square cursor-pointer text-xl text-[#414f63]' onclick='openEditModal(\"{$data['nim']}\", \"{$data['nama_mahasiswa']}\", \"{$data['deskripsi_pelanggaran']}\", \"{$data['tingkat_pelanggaran']}\", \"{$data['id_pelanggaran']}\")'></i>
                                <i class='delete fa-solid fa-trash-can cursor-pointer text-xl text-[#e70000]' onclick='openDeleteModal(\"{$data['id_pelanggaran']}\")'></i>
                            </div>
                        </td>
                    </tr>";
                    $no++;
                ...
                echo "<tr><td colspan='6' class='text-center border-b border-[#e4e8ed]'>Tidak ada data pelanggaran</td></tr>";
            }
        ?>
    </tbody>
</table>
...
<section class="z-50 fixed inset-0 bg-black/50 flex items-center justify-center" id="edit-modal" style="display: none">
    <div class="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-full max-w-lg p-8 flex flex-col items-center rounded-lg shadow-lg bg-white text-center">
        ...
        <form action="/technorules/controller/dosen/pelanggaran.php?method=update" method="post" class="w-full flex flex-col gap-y-4">
...
<!-- Modal Tambah -->
<section class="z-50 fixed inset-0 bg-black/50 flex items-center justify-center" id="add-modal" style="display: none">
    <div class="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-full max-w-lg p-8 flex flex-col items-center rounded-lg shadow-lg bg-white text-center">
        <h3 class="text-2xl font-semibold mb-6 text-gray-800">
            Tambah Data Pelanggaran
        </h3>
        <form action="/technorules/controller/dosen/pelanggaran.php?method=add" method="post" class="w-full flex flex-col gap-y-4">
            <div class="flex flex-col w-full">
...
<section class="z-50 fixed inset-0 bg-black/50 flex items-center justify-center" id="delete-modal" style="display: none">
    <div class="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-full max-w-lg p-8 flex flex-col items-center rounded-lg shadow-lg bg-white text-center">
        ...
        <form action="/technorules/controller/dosen/pelanggaran.php?method=delete" method="post" class="w-full flex flex-col gap-y-4">
...
<script>
function openEditModal(nim, nama, deskripsi, tingkat, id_pelanggaran) {
